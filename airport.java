...
public class airport {
    ...
    public static void makeRunway(runway [] runwayArray){           ///edit to no runway array and one runway?
       ...
       //System.out.println("Enter the runway in use: "); //Check for Valid Runway Input
        ...
        System.out.println("Enter a magnitude for wind. The wind can be as high as 15, or as low as 0.");// Wind Speed
        do {
            knots = in.nextInt() * 1.688;
            WindMagnitude = (int) knots;
        ...
        //runwayArray[1] = new runway(29, 4000, 5264, 4000, 3895.92, 241.23, WindMagnitude, WindDirection);
    }
    ...
    public static void makePlane(plane [] planeArray, runway runwayArray[]){
        ...
        double Vso3 = Vso * 1.3; double Vs3 = Vs * 1.3; double Vfe3 = Vfe * 1.3;double Vx3 = 105; double climbRate3 = 14; double velocity3 = Vfe3 * 1.3;     //faster c172
        double heading = 0;
        ...
        //planeArray[1] = new plane("c150", runwayArray[1].start45X, runwayArray[1].start45Y, velocity2, Vso2, Vs2, Vfe2, Vx2, climbRate2, 29); //plane on other runway
        ...
        while(clock <= 275)
        {
            ...
            if(clock >=1500)   //UAV 20 seconds behind
            {
                ...
                double dd = Math.sqrt( 
                    ...
                    if(dd <= 200)
                    {
                        System.out.println("collision  with " + planeArray[1].planeName + " occurred at clock " + clock + " with distance " + dd);
                        System.exit(1);
                    }
                    System.out.println();
            } 
            ...
            if(clock >=2200)   //other plane 40 seconds behind
            {
                ...
                collide(planeArray, 2, runwayArray[0], clone2);
                ...
                double d = Math.sqrt( 
                    ...
                    if(d <= 200)
                    {
                        System.out.println("collision  with " + planeArray[2].planeName + " occurred at clock " + clock + " with distance " + d);
                        System.exit(1);
                    }
                    System.out.println();
            }
    ...
    public static void changeLeg(Queue legQueue, plane myplane, runway myRunway){
        ...
        if(myplane.collisionMethod == "none" )
        {
            ...
            else if(passed(myplane, myRunway, legQueue) && legQueue.front() == "abeam") // if done with abeam
            {
                ...
                newPoint( myplane.x, myplane.y, myplane.z, myRunway.finalX, myRunway.finalY, 400, d, myplane, myRunway.WindMagnitude, myRunway.WindDirection);
                legQueue.dequeue();
                increasePos(legQueue, myplane, myRunway);
            }
     ...
     // checks each turn direction based off of runway in use
        ...
        { 
            switch (myRunway.runway)
            {
                case 29:
                case 36:
                    if(myPlane.x > myRunway.touchDownX)
                        s = true;
                    break;
                case 11:
                case 18:
    ...
    public static void windCorrection(plane myplane, double WindDirection, double WindMagnitude, double expected, Queue legQueue, runway myRunway){
        ...
        double actual = myplane.heading;
            ...
            if (leg == "downwind" || leg == "abeam"){
                ...
                if (myRunway.runway == 11 && leg == "final"){
                    diff = diff * -1;
                }
    ...
    public static void collide(plane planeArray[], int planeNum, runway myrunway, Queue legqueue)
    {
        ...
        if((planeArray[0].collisionMethod == "none" && d < 1700 && d > 700 && planeArray[0].z > 200) )
        {
        ...
        if(d1 >= 700 && d2>= 700 && planeArray[0].collisionMethod =="ascend")
        {
